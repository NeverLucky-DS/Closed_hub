...
async def start_hr_contact_ref_flow(pool, user_id: int, contact_ref: str) -> tuple[int, str]:
    ...
    reply = (
        f"Записал контакт: {norm}.\n\n"
        "Теперь коротко расскажи про этого человека — можно несколькими сообщениями или голосом.\n"
        "Имеет смысл указать:\n"
        "• где он работает / кого представляет в найме (работодатель отдельной строкой, если знаешь);\n"
